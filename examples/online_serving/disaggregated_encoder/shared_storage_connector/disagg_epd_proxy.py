...
async def process_prefill_stage(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail={
                "error": "Prefill processing error",
                "message": str(e),
            },
        ) from e
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=True,
        help='Comma-separated prefill URLs ("http://p1:8003,http://p2:8004") \
    ...
    if args.prefill_servers_urls.lower() in ("disable", "none", ""):
        app.state.p_urls = []
        logger.info(
            "Disaggregated prefill phase explicitly disabled by user. \
            Running E + PD..."
        )
    else:
        app.state.p_urls = [
        ...
        logger.info(
            "Disaggregated prefill phase is enabled.\
                     Running E + P + D..."
        )
